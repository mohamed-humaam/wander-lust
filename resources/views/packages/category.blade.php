...
@section('content')
        ...
        <div class="container mx-auto pt-44 sm:pt-24 pb-20 sm:pb-6 px-4 relative z-20 h-full flex flex-col justify-end">
            <div
                ...
                <div class="flex items-center mb-4">
                    <span class="bg-orange-500 text-white text-xs font-semibold px-3 py-1 rounded-full">{{ $packages->total() }} Packages</span>
                </div>
                <h1 class="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight mb-4">
                    {{ $category->name }}<br>Packages
                </h1>
                <p class="text-lg text-white/90 mb-6 max-w-2xl">Discover our curated collection of premium {{ strtolower($category->name) }} packages in breathtaking locations worldwide.</p>
    ...
    <div id="packages" class="bg-white dark:bg-gray-900 py-16 sm:py-24">
        <div class="container mx-auto px-4 sm:px-6">
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-12">
                <div class="mb-6 lg:mb-0">
                    <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">
                        Our {{ $category->name }} Packages</h2>
                    <p class="text-gray-600 dark:text-gray-400">{{ $packages->total() }} carefully selected packages</p>
                </div>

                @if($category->children->isNotEmpty())
                    <div class="flex flex-wrap gap-2">
                        @foreach($category->children as $subcategory)
                            <a href="{{ route('packages.category', $subcategory->slug) }}"
            ...
            @if($packages->isEmpty())
                <div class="text-center py-16">
                    ...
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white mb-3">No packages available</h3>
                    <p class="text-gray-500 dark:text-gray-400 max-w-md mx-auto">We're currently updating our collection. Please check back soon for amazing {{ strtolower($category->name) }} packages.</p>
                    <a href="{{ route('packages.index') }}"
                       class="mt-6 inline-flex items-center text-orange-600 dark:text-orange-500 hover:underline font-medium">
                        Browse all packages
            ...
            @else
                ...
                @if($packages->hasPages())
                    <div class="mt-16 flex justify-center">
                        {{ $packages->onEachSide(1)->links('vendor.pagination.tailwind') }}
                    </div>
                @endif
            @endif
        </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-2xl">
                        @if(is_array($category->images) && count($category->images) > 1)
                            <div class="relative h-64 sm:h-96 w-full">
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-gray-900/80 via-gray-900/20 to-transparent"></div>
                            </div>
                        @endif
                            ...
                            <div class="max-w-4xl mx-auto">
                                <div class="mb-10 text-center">
                                    <h2 class="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                                        The {{ $category->name }} Experience</h2>
                                ...
                                <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
                                    <!-- Feature 1 -->
                                    <div
                                        class="group relative bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
                                        <div
                                            class="absolute -top-5 left-6 w-12 h-12 bg-orange-500 rounded-lg flex items-center justify-center text-white transform group-hover:-translate-y-2 transition-all duration-300">
                                        ...
                                        <h3 class="mt-8 text-xl font-semibold text-gray-900 dark:text-white">Premium
                                            Comfort</h3>
                                        <p class="mt-4 text-gray-600 dark:text-gray-400">Experience unparalleled comfort
                                            with our carefully selected accommodations featuring premium amenities and
                                            thoughtful design.</p>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <h3 class="mt-8 text-xl font-semibold text-gray-900 dark:text-white">Prime
                                            Locations</h3>
                                        <p class="mt-4 text-gray-600 dark:text-gray-400">Our properties are
                                            strategically located to provide easy access to key attractions and business
                                            districts.</p>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <h3 class="mt-8 text-xl font-semibold text-gray-900 dark:text-white">Trusted
                                            Quality</h3>
                                        <p class="mt-4 text-gray-600 dark:text-gray-400">Every property undergoes
                                            rigorous vetting to ensure they meet our high standards of quality and
                                            service.</p>
                                    </div>
                                </div>
                                ...
                                <div class="mt-16 text-center">
                                    <a href="#packages"
                                       class="inline-flex items-center px-8 py-4 border border-transparent text-base font-medium rounded-full shadow-sm text-white bg-orange-600 hover:bg-orange-700 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500">
                                        Explore {{ $category->name }} Packages
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-2" viewBox="0 0 20 20"
                                             fill="currentColor">
                                            <path fill-rule="evenodd"
